...
\newminted{cpp}{gobble=2,linenos=0, mathescape, numbersep=3pt, frame=lines, framesep=2mm, baselinestretch=1, fontsize=\scriptsize}
\newmintinline{cpp}{}

\newminted{bash}{gobble=2,linenos=0, mathescape, numbersep=3pt, frame=lines, framesep=2mm, baselinestretch=1, fontsize=\scriptsize}
\newmintinline{bash}{}
...
\abstract{%
We will present our results and challenges with C++ modules in ROOT. ROOT was extended with experimental support for using C++ modules during runtime, with aims to reduce its memory usage and improve its correctness. C++ Modules is a mechanism to boost  compilation time. Compilation time for other software turns into runtime for ROOT as it's using Cling C++ interpreter. 
...
%...for example, ROOT’s {\it libGeom} and other third-party code.

%This is done to circumvent the costly call to {\it ShowMembers} which will require parsing.
...
\begin{listing}[h]
    ...
    \end{cppcode*}
    \end{minipage}
    \caption{C++ header {\it Foo.h} for {\it libFoo} library, {\it libFoo.rootmap} written for libFoo library, and {\it G\_\_Foo.cxx} generated to insert forward declaration to ROOT when the libFoo library was loaded.}
    \label{list:foo}
\end{listing}
...
\begin{listing}[h]
    ...
    \end{cppcode*}
    \end{minipage}
    \caption{Example of the efforts which ROOT does to avoid parsing redundant code. Consider {\it foo::bar} and {\it S} are defined in {\it libFoo}'s {\it Foo.h} shown in Listing \ref{list:foo}.}
    \label{list:prompt}
\end{listing}
